...
export const CAMPAIGN_LIST_QUERY = `query CampaignList($input: ListCampaignInput!) {
...
async function postGalxeGraphql(payload, defaultMessage) {
  try {
    const response = await axios.post(GALXE_ENDPOINT, payload, {
      headers: {
        accept: '*/*',
    ...
    throw new Error(getAxiosErrorMessage(error, defaultMessage))
  }
}
...
export async function queryCampaignWinners(taskUrl, addressTexts = {}) {
  ...
  const result = await postGalxeGraphql(
    {
...
export async function queryCampaignList(input) {
  const result = await postGalxeGraphql(
    {
